...
class AccountSettingsRepository @Inject constructor(
    private val dao: AccountSettingsDao,
) {
    fun observe(accountId: String): Flow<AccountSettings> =
        dao.observe(accountId).map { it?.toDomain() ?: AccountSettings(accountId) }

    suspend fun get(accountId: String): AccountSettings =
        dao.get(accountId)?.toDomain() ?: AccountSettings(accountId)
    ...
    suspend fun setSignature(accountId: String, signature: String) =
        update(accountId) { it.copy(signature = signature) }

    suspend fun setSignatureEnabled(accountId: String, enabled: Boolean) =
        update(accountId) { it.copy(signatureEnabled = enabled) }

    suspend fun setNotificationsEnabled(accountId: String, enabled: Boolean) =
        update(accountId) { it.copy(notificationsEnabled = enabled) }
