...
class MailRepositoryImpl @Inject constructor(
    ...
    // Room is the single source of truth. Until sync lands the cache is empty, so
    // we fall back to bundled sample data to keep the UI populated.
        ...
        accountDao.observeAll().map { rows ->
            rows.map { row ->
                Account(
                    ...
                    authType = runCatching { AuthType.valueOf(row.authType) }
                        .getOrDefault(AuthType.PASSWORD_IMAP),
                )
            }
        }
